window.__ModuleLoader__.load({ id: "dsh-terminal-skin", factory: (require) => {
var module = { exports: {} };
...
function injectStyle() {
    if (document.getElementById('dsh-terminal-skin-style'))
        return;
...
function injectWhale() {
    if (document.getElementById('dsh-terminal-whale-host'))
        return;
...
function getStoredWidthMode() {
    try {
        return localStorage.getItem(WIDTH_KEY) === 'wide' ? 'wide' : 'center';
    }
    catch {
        return 'center';
    }
...
function injectWidthToggle() {
    if (document.getElementById('dsh-terminal-width-toggle'))
        return;
    const make = () => {
