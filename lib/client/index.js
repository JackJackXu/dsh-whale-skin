...
const TERMINAL_CSS = [
    ...
    // User bubble: the colour block wraps ONLY the text — 12px left padding
    ...
    // side runs to the column edge (no right padding), so user-message text
    // ends flush with assistant text instead of stopping 12px short.
    ...
    // Full opacity: the whale is a solid white tile (see whale.ts) — dimming it
    // would make the white bleed into the theme background.
    ...
    // user + steering + pending actions get the same 12px LEFT inset as the
    // bubbles, and the right side runs to the column edge (no right inset), so
    // the grey card ends flush with the user bubble above it. (Pending steering
    // becomes a REAL steering node on send, so both the data-chat-flow-kind=
    // "steering" and the pre-admission attribute must match — a missing variant
    // is exactly what "drifts left after sending".)
    ...
    // scroll padding (44px), 6px top/bottom, left 12px + right flush to the
    // column edge (text ends with the assistant's).
...
function fixBottomBar() {
    ...
    if (dock) {
        ...
        if (hasContent && dock.parentNode !== bottomBar)
        ...
        else if (!hasContent && dock.parentNode === bottomBar)
